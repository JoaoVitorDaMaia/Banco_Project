...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div style={{ background: "#1a1a2e", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 8, padding: "8px 14px" }}>
        <div style={{ fontSize: 11, color: "#888", marginBottom: 2 }}>{label}</div>
        <div style={{ fontSize: 14, fontWeight: 700, color: "#c7d2fe" }}>{formatCurrency(payload[0].value)}</div>
      </div>
    );
  }
  return null;
  ...
  const sendMoney = () => {
    ...
    setTimeout(() => {
      setTransactions(prev => [{
        name: recipient, type: "Transferência",
        value: -num, date: new Date().toLocaleDateString("pt-BR"),
      }, ...prev]);
      setRecipient(""); setAmount(""); setSending(false);
    }, 600);
  };
    ...
    <div style={s.root}>
      ...
      <nav style={s.sidebar} className="sidebar">
        ...
        <div style={s.navGroup}>
          <SideNavItem label="Início" active={page === "dashboard"} onClick={() => navigate("dashboard")} />
          <SideNavItem label="Transações" active={page === "transactions"} onClick={() => navigate("transactions")} />
        </div>
        <div style={s.sidebarUser}>
          <div style={s.userAvatar}>VC</div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#f0f0f0" }}>Você</div>
      ...
      <header style={s.topbar} className="topbar">
        ...
        <button style={s.menuBtn} onClick={() => setMenuOpen(o => !o)} aria-label="Menu">
          <span style={{ ...s.hbar, transform: menuOpen ? "rotate(45deg) translate(5px,5px)" : "none" }} />
      ...
      {menuOpen && (
        <div style={s.drawer}>
          <SideNavItem label="Início" active={page === "dashboard"} onClick={() => navigate("dashboard")} drawer />
          <SideNavItem label="Transações" active={page === "transactions"} onClick={() => navigate("transactions")} drawer />
        </div>
      )}
      ...
      <nav style={s.bottomNav} className="bottom-nav">
        <BotNavItem label="Início" active={page === "dashboard"} onClick={() => navigate("dashboard")} />
        <BotNavItem label="Transações" active={page === "transactions"} onClick={() => navigate("transactions")} />
      </nav>
    </div>
  );
}
...
function Dashboard({ totalBalance, totalExpense, transactions, recipient, setRecipient, amount, setAmount, sendMoney, sending, errorMsg }) {
    ...
    <div style={s.dashContent}>
      ...
      <div style={s.heroRow} className="hero-row">
        <div style={{ ...s.balanceCard, flex: 2 }} className="balance-card">
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            {noFunds && (
              <div style={s.noFundsBadge}>Saldo esgotado</div>
            )}
          </div>
          <div style={s.balanceDeco}>
            <svg width="200" height="110" viewBox="0 0 220 120" fill="none" style={{ opacity: 0.07 }}>
        ...
        <div style={{ ...s.card, flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", gap: 6 }}>
          <div style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: "0.06em" }}>Total gasto</div>
          <div style={{ fontSize: "clamp(20px,3vw,28px)", fontWeight: 800, color: "#f87171", letterSpacing: "-0.5px" }}>
            {formatCurrency(totalExpense)}
          </div>
          <div style={{ fontSize: 12, color: "#444" }}>{transactions.filter(t => t.value < 0).length} transações</div>
        </div>
      </div>
      ...
      <div style={s.midRow} className="mid-row">
        <div style={s.card}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={CHART_DATA} barSize={22} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              <XAxis
                dataKey="mes"
        ...
        <div style={s.card}>
          ...
          {noFunds ? (
            <div style={s.noFundsBox}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: "#f87171", marginBottom: 4 }}>Saldo insuficiente</div>
            ...
            <>
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {CONTACTS.map(c => (
                    <button key={c.name} onClick={() => setRecipient(c.name)} className="contact-btn"
                      style={{ ...s.contactBtn, ...(recipient === c.name ? s.contactActive : {}) }}>
                      <div style={{
                        ...s.contactAv,
                        background: recipient === c.name ? "linear-gradient(135deg,#6366f1,#818cf8)" : "rgba(255,255,255,0.08)",
                        color: recipient === c.name ? "#fff" : "#aaa",
                      }}>{c.initials}</div>
                      <span style={{ fontSize: 10, color: recipient === c.name ? "#c7d2fe" : "#555" }}>{c.name}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={s.amountWrap}>
                  <span style={{ fontSize: 13, color: "#555", marginRight: 6 }}>R$</span>
                  <input type="text" value={amount} onChange={e => setAmount(formatAmountInput(e.target.value))}
                    placeholder="0,00" style={s.amountInput} inputMode="numeric" />
                </div>
              </div>
              ...
              <button onClick={sendMoney} disabled={!recipient || !amount || sending} className="send-btn"
                style={{ ...s.sendBtn, opacity: (!recipient || !amount) ? 0.4 : 1 }}>
      ...
      <div style={s.card}>
        <div style={s.cardHeader}>
          ...
          <span style={{ fontSize: 12, color: "#6366f1", cursor: "pointer" }}>Ver todas</span>
        </div>
        {transactions.length === 0 ? (
          ...
          transactions.slice(0, 4).map((t, i) => (
            <div key={i} style={{
              ...s.txRow,
              borderBottom: i < Math.min(3, transactions.length - 1) ? "1px solid rgba(255,255,255,0.05)" : "none",
            }}>
              <div style={{
                ...s.txAv,
                background: t.value > 0 ? "rgba(74,222,128,0.12)" : "rgba(248,113,113,0.12)",
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={s.txMeta}>{t.type} · {t.date}</div>
              </div>
              <div style={{
                fontSize: 14, fontWeight: 700, letterSpacing: "-0.3px",
                color: t.value > 0 ? "#4ade80" : "#f87171", flexShrink: 0,
              }}>
...
const s = {
  root: {
    ...
    color: "#e8e8f0", position: "relative",
  },
  sidebar: {
  ...
  hbar: {
    ...
    background: "#aaa", borderRadius: 2, transition: "all 0.2s",
  },
  drawer: {
  ...
  amountInput: {
    ...
    color: "#f0f0ff", letterSpacing: "-0.3px", fontFamily: "inherit",
  },
  errorMsg: {
